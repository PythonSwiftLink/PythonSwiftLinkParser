...
public extension WrapFunction {
    ...
    func generate(PyMethod_withArgs cls_title: String?) -> String {
        ...
        .init(withArgs: "\(name)") { s, args, nargs in
            do {
                guard nargs > \(_args_.count - 1), let args = args else { throw PythonError.call }
                \(arg_extract)
                \(completion_handlers)
    ...
    private func generate(extractLine arg: WrapArgProtocol, many: Bool = false) -> String {
        ...
        if many {
            ...
            if arg.type == .callable {
                ...
                return "let _\(arg.name) = args[\(arg.idx)]"
            }
            if arg.type == .object {
                return "let \(arg.name) = args[\(arg.idx)]"
            }
            if arg.type == .other {
                return """
                let \(arg.name) = args[\(arg.idx)]
            ...
            return "let \(arg.name) = try \(_arg)(object: args[\(arg.idx)])"
        }
        if arg.type == .callable {
